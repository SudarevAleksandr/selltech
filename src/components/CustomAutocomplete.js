...
class CustomAutocomplete extends React.Component {
    ...
    constructor(props) {
        super(props);
        this.state = {focus: false, value:'', searchValue:'', userItem:'', filteredList: this.getFilteredList('')};
    ...
    open = ()=> {
      this.setState({focus:true, filteredList: this.getFilteredList(this.state.searchValue)});
    } 
    ...
    render() {
      ...
      >   
            <TextField 
             ...
             />
              {this.state.focus ? (
                  <div className='CustomAutocomplete-listWrap'>
                      <Listbox > 
                          {this.state.filteredList.map((item) => (
                              <MenuItem key={item.id} onClick={() => this.selectItem(item)}> 
                                    {item.name}
                                    {item.removable ? 
                                        <button className='deleteButton' onClick={(e) => this.deleteItem(e, item)}>
